...
{
    [Serializable]
    public partial class ReusableTween
    {
#if UNITY_EDITOR
        ...
        [ShowInInspector]
        [SerializeField]
        internal UnityEngine.Object unityTarget;
        ...
        internal void setPropType(
            [System.Diagnostics.CodeAnalysis.SuppressMessage("ReSharper", "UnusedParameter.Global")] PropType value)
        {
        }
#endif
        [SerializeField]
        internal ValueContainerStartEnd startEndValue;

        internal PropType getPropType() =>
            Utils.TweenTypeToTweenData(startEndValue.tweenType).Item1; // todo rename to propType
        ...
        [SerializeField]
        internal TweenSettings settings;
        ...
        internal void updateSequence(float _elapsedTimeTotal, bool isRestart, bool earlyExitSequenceIfPaused = true)
        {
            ...
            if (cyclesDiff != 0 && !isRestartToBeginning)
            {
                ...
                for (int i = 0; i < cyclesDiffAbs; i++)
                {
                    ...
                    if (sequenceCycleMode == CycleMode.Restart && cyclesDone != settings.cycles &&
            ...
            bool isEarlyExitAfterChildUpdate()
            {
                ...
                return earlyExitSequenceIfPaused &&
                       sequence.root.tween
                           ._isPaused; // access isPaused via root tween to bypass the cantManipulateNested check
            }
        }
        ...
        internal void OnComplete<T>([CanBeNull] T _target, [NotNull] Action<T> _onComplete, bool warnIfTargetDestroyed)
            where T : class
        {
            if (_target == null || isDestroyedUnityObject(_target))
            {
                Debug.LogError(
                    $"{nameof(_target)} is null or has been destroyed. {Constants.onCompleteCallbackIgnored}");
                return;
            }
        ...
        void handleOnCompleteException(Exception e)
        {
            ...
            Assert.LogError(
                $"Tween's onComplete callback raised exception, tween: {GetDescription()}, exception:\n{e}\n", id,
                target as UnityEngine.Object);
        }

        [System.Diagnostics.CodeAnalysis.SuppressMessage("ReSharper", "ConditionIsAlwaysTrueOrFalse")]
        static bool isDestroyedUnityObject<T>(T obj) where T : class =>
        ...
        internal void Setup([CanBeNull] object _target, ref TweenSettings _settings,
            [NotNull] Action<ReusableTween> _onValueChange, [CanBeNull] Func<ReusableTween, ValueContainer> _getter,
            bool _startFromCurrent, TweenType _tweenType)
        {
        ...
        void ReportOnValueChange(float _easedInterpolationFactor)
        {
            ...
            if (startFromCurrent)
            {
                ...
                {
                    Assert.LogWarning(
                        $"Tween's 'endValue' equals to the current animated value: {startValue.Vector4Val}, tween: {GetDescription()}.\n" +
        ...
        internal Quaternion QuaternionVal => Quaternion.SlerpUnclamped(startValue.QuaternionVal, endValue.QuaternionVal,
            easedInterpolationFactor);
        ...
        void invokeOnUpdate<T>() where T : class
        {
            ...
            {
                Assert.LogError(
                    $"OnUpdate() will not be called again because it thrown exception, tween: {GetDescription()}, exception:\n{e}",
                    id, target as UnityEngine.Object);
                clearOnUpdate();
            }
        }
